...
The policy is generic, but two websites do not establish broad reliability. Name resolution covers common labels, root-local ARIA references, and text; it is not the browser's full accessibility algorithm. Open shadow roots and accessible same-origin frames are traversed; closed roots, cross-origin/opaque frames, padded or transformed/zoomed frame chains, canvas, uploads, nested scrolling, pop-ups, and complex keyboard interactions remain unsupported. A valid action can still be wrong. Independent checks, rather than the model's DONE choice, determine whether the demonstrated task succeeded.
