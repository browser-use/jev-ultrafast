...
Sixty browser actions and 120 decision requests bound a run. A caller can tighten either bound and add a wall-clock budget for one run; neither can be raised past the standing limit. The time budget is checked before a model call and again before the action that call chose, so an expired run neither starts a cycle it cannot finish nor mutates the page after its deadline. It is measured from the first decision, so tab creation and the first observation sit outside it, as they do in the published timings. Every stop records a reason: a model choice, a spent budget, or three executed actions that changed nothing. Up to 250 action candidates are retained; truncated candidates cannot be selected. The service stays loopback-only, serializes inspector actions, and checks Host, Origin, and a local request token. The MCP server is goal-level: its tools accept a URL and a goal, never an element index, selector, coordinate, or script, and it serializes calls against one browser rather than queueing them. Credentials remain server-side. Tabs share the existing Chrome profile.
